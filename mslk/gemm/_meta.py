...
"""Python-side Meta (shape inference) and Autograd implementations for GEMM ops.
...
Autograd (backward) support for FP4 GEMM ops is also registered here.
...
# ---------------------------------------------------------------------------
# FP4 dequantization helpers (used by autograd backward)
# ---------------------------------------------------------------------------
...
def _dequantize_mxfp4_to_bf16(
    ...
    # Unpack nibbles to float32 via LUT
...
def _dequantize_fp4_ultra(
    ...
    # Unpack nibbles to float
    x_float = fp4_to_float(xq_u8)  # [M, K] or [..., K]
    ...
    # local_scale as float: fp8 → float
    local_scale = scale_fp8.view(torch.float8_e4m3fn).to(torch.float32)

    # true_scale = local_scale * global_scale_inv
...
if hasattr(torch.ops.mslk, "f4f4bf16"):
    ...
    def _f4f4bf16_backward(ctx, grad_output):  # type: ignore[no-untyped-def]
        ...
        # 7 inputs: XQ, WQ, x_scale, w_scale, output, global_scale, mxfp4_block_size
...
if hasattr(torch.ops.mslk, "f4f4bf16_grouped_mm"):
    ...
    def _f4f4bf16_grouped_mm_backward(ctx, grad_output):  # type: ignore[no-untyped-def]
...
if hasattr(torch.ops.mslk, "f4f4bf16_ultra_grouped_mm"):
    ...
    def _f4f4bf16_ultra_backward(ctx, grad_output):  # type: ignore[no-untyped-def]
        XQ, WQ, x_scale, w_scale, offsets = ctx.saved_tensors
